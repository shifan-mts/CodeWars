...
const questionBank = {
  gun1: [
    ["int x = 5;", 'printf("%d", x + 3);', "// What will be printed?"],
...
function getQuestionForGun(gun, availableQuestions) {
  if (availableQuestions[gun].length === 0) {
    availableQuestions[gun] = [
      [
        "You are exhausted of this level difficulty questions. Kindly move on to the next or previous difficulty level.",
      ],
    ]; // To stop looping of the questions
  }

  const randomIndex = Math.floor(
    Math.random() * availableQuestions[gun].length
  );
    ...
    if (data.type === "shoot") {
      ...
      if (!shooter.canShoot) {
        ws.send(
          JSON.stringify({
            type: "error",
            message: "You must answer correctly before shooting!",
          })
        );
        return;
      }
